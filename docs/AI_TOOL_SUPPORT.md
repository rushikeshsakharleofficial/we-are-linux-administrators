...
`linux-admin` keeps one canonical 76-skill tree under `skills/` and uses thin adapters or native Agent Skills discovery instead of vendor-specific copies.
...
6. Never duplicate all 76 skills into vendor-specific folders merely to advertise support.
...
Converted parent/chunk domains include network, time, storage mounts/filesystem/SMART/quota, performance, permissions, auth, logging, automation Bash/runbook work, post-containment incident RCA, and security host-audit/auditd/Fail2Ban work. Distinct high-risk/product-specific specialists remain separate when that improves routing accuracy.
