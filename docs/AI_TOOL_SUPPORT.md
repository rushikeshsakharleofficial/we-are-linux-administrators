...
`linux-admin` keeps one canonical 85-skill tree under `skills/` and uses thin adapters or native Agent Skills discovery instead of vendor-specific copies.
...
6. Never duplicate all 85 skills into vendor-specific folders merely to advertise support.
...
Converted parent/chunk domains now include network, time, storage baseline branches, performance, permissions, and auth. `auth` routes local accounts, PAM, SSSD/LDAP and sudoers to focused chunks while SSH hardening remains separate because remote-access changes have a distinct lockout/security blast radius.
