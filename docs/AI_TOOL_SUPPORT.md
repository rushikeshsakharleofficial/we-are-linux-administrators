...
`linux-admin` keeps one canonical 72-skill tree under `skills/` and uses thin adapters or native Agent Skills discovery instead of vendor-specific copies.
...
6. Never duplicate all 72 skills into vendor-specific folders merely to advertise support.
...
Converted parent/chunk domains include network, time, storage mounts/filesystem/SMART/quota/LVM/RAID/iSCSI, performance, permissions, auth, logging, automation Bash/runbook work, package lifecycle/patch rollout, post-containment incident RCA, and security host-audit/auditd/Fail2Ban work. Multipath, NFS, Samba, backup/restore and other distinct high-risk/product-specific specialists remain separate when that improves routing accuracy.
