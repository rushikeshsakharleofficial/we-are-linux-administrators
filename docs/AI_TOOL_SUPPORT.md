...
`linux-admin` keeps one canonical 77-skill tree under `skills/` and uses thin adapters or native Agent Skills discovery instead of vendor-specific copies.
...
6. Never duplicate all 77 skills into vendor-specific folders merely to advertise support.
...
Converted parent/chunk domains include network, time, storage baseline branches, performance, permissions, auth, logging, automation Bash/runbook work, post-containment incident RCA, and security host-audit/auditd/Fail2Ban work. The former broad `os-security-expert` is consolidated into `security-expert/chunks/security-audit.md`; distinct high-risk/product-specific specialists remain separate when that improves routing accuracy.
