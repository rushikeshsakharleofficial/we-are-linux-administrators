...
| GitHub Copilot CLI | `AGENTS.md`, `.github/copilot-instructions.md`, `.github/instructions/*.instructions.md` | `$HOME/.copilot/copilot-instructions.md`, `$HOME/.copilot/instructions/**/*.instructions.md` | Keep repository adapter thin; do not copy 82 skills into Copilot-specific folders. |
...
1. `linux-admin status` reports the expected version, source path and current top-level skill count (`82` for repository metadata `1.18.7`).
...
The compact parent/chunk structure includes auth routing for local accounts, PAM, SSSD/LDAP and sudoers; logging routing for rsyslog and logrotate; and post-containment root-cause analysis under `incident-response-expert`. SSH hardening, product-specific monitoring, and formal incident artifact generation remain separate top-level specialists where their risk or tool surface differs.
