...
1. `linux-admin status` reports repository metadata `1.18.10` and current top-level skill count `77`.
...
The compact tree now includes parent/chunk routing for network, time, storage baseline, performance, permissions, auth, logging, automation Bash/runbook work, incident RCA, and security host-audit/auditd/Fail2Ban branches. The broad OS-security audit is part of `security-expert/chunks/security-audit.md`; distinct high-risk or product-specific specialists remain separate when merging would reduce safety or routing accuracy.