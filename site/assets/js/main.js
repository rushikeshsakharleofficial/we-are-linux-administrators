...
(function () {
  ...
  const PROJECT_VERSION = '1.18.6';
  const SKILL_COUNT = '83';
  const REPO = 'rushikeshsakharleofficial/we-are-linux-administrators';
  const STALE_COUNTS = new Set(['46', '85', '89', '91', '95', '98', '99', '101', '102', '103', '106', '107', '108']);
  const REMOVED_SKILLS = new Set([
    ...
    'rsyslog-expert', 'logrotate-expert'
  ]);
  ...
  function replaceText(text) {
    return text
      .replace(/\b(?:46|85|89|91|95|98|99|101|102|103|106|107|108)\s+(?=skills\b)/gi, `${SKILL_COUNT} `)
      .replace(/\b(?:46|85|89|91|95|98|99|101|102|103|106|107|108)\s+(?=Expert Skills|Focused Skills\b)/gi, `${SKILL_COUNT} `)
      .replace(/\bBrowse all (?:35|40\+|46|85|89|91|95|98|99|101|102|103|106|107|108)(?=\s+skills?\b)/gi, `Browse all ${SKILL_COUNT}`)
  ...
  document.querySelectorAll('a[href*="/releases/tag/"]').forEach(link => {
    if (/v1\.17\.18|v1\.17\.75|v1\.18\.[0-6]/.test(link.textContent + link.href)) {
